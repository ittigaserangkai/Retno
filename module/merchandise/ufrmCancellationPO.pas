...
type
  TfrmCancellationPO = class(TfrmMasterBrowse)
    pnlTop: TPanel;
    lbl1: TLabel;
    edPONOAwal: TEdit;
    lbl2: TLabel;
...
var
  frmCancellationPO: TfrmCancellationPO;
...
procedure TfrmCancellationPO.FormClose(Sender: TObject;
...
procedure TfrmCancellationPO.FormDestroy(Sender: TObject);
begin
  inherited;
  frmCancellationPO := nil;
end;

procedure TfrmCancellationPO.FormCreate(Sender: TObject);
begin
  inherited;
  lblHeader.Caption := 'CANCELLATION PURCHASING ORDER';
end;

procedure TfrmCancellationPO.actEditExecute(Sender: TObject);
var sSQL: String;
  IsCheck: Boolean;
  SS: TStrings;
    countData, i: Integer;
...
procedure TfrmCancellationPO.btnShowPOClick(Sender: TObject);
begin
  ShowPOByRangeNumber(edPONOAwal.Text, edPONOAkhir.Text);
end;

procedure TfrmCancellationPO.ShowPOByRangeNumber(AFirstNumber,
  ALastNumber: string);
var
  i: integer;
...
begin
    ...
    sSQL := ' select a.po_no '
         + ' from po a '
         + ' where '
         + ' a.po_stapo_id in (2, 4, 6) '
         + ' and a.is_po_bonus = 0 '
         + ' and a.po_unt_id = '+IntToStr(FLoginUnitId)+' '
         + sFilterNoPO;
  {
     ...
     try
        ...
        begin
          ...
          begin
            ...
            FPO := TPO.Create(nil);
            try
              if FPO.LoadByNoBukti(fieldByName('po_no').AsString,FLoginUnitId) then
              begin
                with strgGrid do
                begin
...
procedure TfrmCancellationPO.ParseHeaderGrid();
begin
  {with strgGrid do
  begin
    Clear;
...
procedure TfrmCancellationPO.strgGridCanEditCell(Sender: TObject; ARow,
...
procedure TfrmCancellationPO.strgGridCheckBoxClick(Sender: TObject; ACol,
...
procedure TfrmCancellationPO.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  i: Integer;
  _isChecked: Boolean;
...
procedure TfrmCancellationPO.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TfrmCancellationPO.actRefreshExecute(Sender: TObject);
begin
  inherited;
  ShowPOByRangeNumber('','');
end;

procedure TfrmCancellationPO.FormActivate(Sender: TObject);
begin
  inherited;
  frmCancellationPO.Caption:= 'CANCELLATION PO';
end;

procedure TfrmCancellationPO.strgGridGetFloatFormat(Sender: TObject; ACol,
...
procedure TfrmCancellationPO.edPONOAwalKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  sSQL: string;
begin
  ...
  begin
    sSQL := 'select PO_NO as "NOMOR PO", PO_DATE as "TGL PO"'
            + ' from po'
            + ' where po_unt_id = ' + IntToStr(FLoginUnitId)
            + ' and po_stapo_id in (2, 4, 6)';
    {
...
procedure TfrmCancellationPO.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
