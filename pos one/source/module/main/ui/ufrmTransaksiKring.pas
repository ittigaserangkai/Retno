...
uses
  ...
  Dialogs, StdCtrls, Grids, BaseGrid, AdvGrid, AdvCGrid, EnhEdit, ExtCtrls,
  AdvEdit, uRMSUnit;
...
type
  TfrmTransaksiKring = class(TForm)
    ...
    Label4: TLabel;
    edNomorPO: TAdvEdit;
    edNamaPelanggan: TAdvEdit;
    edNomorDO: TAdvEdit;
    tmrInfo: TTimer;
    Label3: TLabel;
    edNoTrnTerakhir: TEnhancedEdit;
    Label5: TLabel;
    edNoPelanggan: TAdvEdit;
...
uses
  uDOAssgros, ufrmMain, uConstanta, Math, ufrmPaymentKring, ufrmTransaksi,
  ufrmPayment, uNewMembership, uNewAssgros, uDiscountMember, StrUtils;
      ...
      Result := Result + (Columns[_KolTotal].Floats[i]);  //Ceil
{
      ...
      begin
        TotalRupiahBarangAMC := TotalRupiahBarangAMC + Ceil(Columns[_KolTotal].Floats[i]);
      end;
}
...
procedure TfrmTransaksiKring.sgTransaksiGetAlignment(Sender: TObject; ARow,
...
begin
  ...
  begin
    if ACol in [0,_KolJumlah,_KolHarga,_KolDisc,_KolTotal] then
...
begin
  ...
  else if (Key in [VK_F9]) and (Trim(sgTransaksi.Cells[_KolPLU, sgTransaksi.FixedRows]) <> '') then
  begin
    ShowPayment;
  end;
end;
