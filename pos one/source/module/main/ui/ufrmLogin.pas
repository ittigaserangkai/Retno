...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, AdvEdit, advlued, ExtCtrls;
...
type
  TfrmLogin = class(TForm)
    ...
    Label2: TLabel;
    sbLogin: TStatusBar;
    edCashierID: TAdvLUEdit;
    edNama: TAdvLUEdit;
    edPassword: TAdvLUEdit;
    tmrLogin: TTimer;
...
uses
  uRMSUnit, ufrmMain, uGTSUICommonDlg, DB;
  ...
  case Key of    //
    ...
    begin
      ...
      sSQL := 'select a.usr_fullname, a.usr_passwd, a.usr_id, b.balance_id '
        ...
        + '  and a.usr_username = ' + Quot(edCashierID.Text)
        + '  and a.usr_unt_id = ' + IntToStr(frmMain.UnitID)
        + '  and b.balance_shift_date = ' + QuotD(dServerDate)
        + '  and b.BALANCE_STATUS = ' + Quot('OPEN')
        + ' inner join setuppos c on b.balance_setuppos_id = c.setuppos_id '
        + '  and b.balance_setuppos_unt_id = c.setuppos_unt_id '
        + '  and c.setuppos_terminal_code = ' + Quot(frmMain.FPOSCode)
        + '  and c.setuppos_is_active = 1 '
        + ' inner join shift d on b.balance_shift_id = d.shift_id '
        + '  and b.balance_shift_unt_id = d.shift_unt_id '
        + '  and cast(d.shift_start_time as time) <= ' + Quot(FormatDateTime('hh:nn:ss', dServerDate))
        + '  and cast(d.shift_end_time as time) >= ' + Quot(FormatDateTime('hh:nn:ss', dServerDate));
