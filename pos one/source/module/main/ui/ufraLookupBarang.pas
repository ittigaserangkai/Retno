...
uses
  ...
  Dialogs, StdCtrls, Grids, BaseGrid, AdvGrid, AdvCGrid, ExtCtrls, DBAdvGrd,
  AdvProgressBar, DB, IBCustomDataSet, IBQuery, Menus;

type
  TfraLookupBarang = class(TFrame)
    sgBarang: TAdvColumnGrid;
    pnlHeader: TPanel;
    edNamaBarang: TEdit;
    pbBarang: TAdvProgressBar;
    ...
    procedure sgBarangRowChanging(Sender: TObject; OldRow, NewRow: Integer;
      var Allow: Boolean);
...
begin
  ...
  begin
    sgBarang.Col := _KolNamaBarang;
    sgBarang.SetFocus;
    if sgBarang.RowCount <= 1 then exit;
  end
...
begin
  PLU := '';
  UOM := '';
	if sgBarang.Row = 0 then Exit;
  if pbBarang.Position < 100 then Exit;

  PLU          := sgBarang.Cells[_KolPLU, sgBarang.Row];
  UOM          := sgBarang.Cells[_KolSatuan, sgBarang.Row];
  Self.Visible := False;
...
procedure TfraLookupBarang.sgBarangKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if ((Key = VK_UP) or (Key = 33)) and (sgBarang.Row = 1) and edNamaBarang.Enabled then
  begin
  ...
  if ((Key = VK_UP) or (Key = 33)) and (sgBarang.Row = 1) then
  begin
    frmTransaksi.sgTransaksi.SetFocus;
    Exit;
  end
  else
  if ((Key = VK_DOWN) or (Key = 34)) and (sgBarang.Row = (sgBarang.RowCount-1))
...
procedure TfraLookupBarang.sgBarangRowChanging(Sender: TObject; OldRow,
