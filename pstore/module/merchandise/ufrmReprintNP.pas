...
uses
  ...
  cxLookAndFeelPainters, Vcl.Menus, cxButtons, System.Actions, cxStyles,
  cxClasses;
...
type
  TfrmReprintNP = class(TfrmMaster)
    ...
    btnSearch: TcxButton;
    btnReprintNP: TcxButton;
    btnCheckListPO: TcxButton;
    cxShowDetailPO: TcxButton;
...
uses uDMClient, uTSCommonDlg, uAppUtils, ufrmSearchPO, uConstanta,
   ufrmDisplayPO, uRetnoUnit; //ufrmDialogPrintPreview, ufrmDeliveryOrder,
...
procedure TfrmReprintNP.btnSearchClick(Sender: TObject);
begin
...
begin
  ...
  if (edt1.Text <> '') then begin
    if (CommonDlg.Confirm('Are you sure you want to print check list NP (PO NO. : '+ edt1.Text +')?') = mrYes) then begin
      ...
      //ShowReport('frCheckListText', s, '', True);
