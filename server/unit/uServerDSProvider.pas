...
type
  ...
  public
    ...
    function POTrader_SlipByID(aID: string): TFDJSONDataSets;
...
function TDSProvider.ReturTrader_GetDSOverview(ATglAwal , ATglAkhir : TDateTime; AUnit :
        TModUnit = nil): TDataset;
var
  sSQL: string;
begin
  sSQL := 'SELECT * FROM V_RETUR_TRADER' +
          ' WHERE RET_DATE BETWEEN ' + TDBUtils.QuotDt(StartOfTheDay(ATglAwal)) +
          ' AND ' + TDBUtils.QuotDt(EndOfTheDay(ATglAkhir));

  if AUnit <> nil then
    sSQL := sSQL + ' and RET_UNIT_ID = ' + QuotedStr(AUnit.ID);
...
function TDSProvider.DOTrader_GetLookupForRetur(ATglAwal, ATglAkhir: TDateTime;
    AUnitID: String): TDataset;
var
  sSQL: string;
begin
  ...
  sSQL := 'select * from V_DOTRADER where IsNull(DOT_STATUS,'''') <> ''RETURNED'' ';

  sSQL := sSQL + ' and DOT_DATE between ' + TDBUtils.QuotDt(ATglAwal)
        +' and ' + TDBUtils.QuotDt(ATglAkhir);

  if AUnitID <> '' then
    sSQL := sSQL + ' and DOT_UNIT_ID = ' + QuotedStr(AUnitID);

  sSQL := sSQL + ' order by DOT_NO';
...
function TDSReport.POTrader_SlipByID(aID: string): TFDJSONDataSets;
var
  S: string;
begin
  Result := TFDJSONDataSets.Create;
  S := 'select * from V_POTRADER_SLIP where POTRADER_ID = ' + QuotedStr(aID);
  TFDJSONDataSetsWriter.ListAdd(Result, TDBUtils.OpenQuery(S));
end;
