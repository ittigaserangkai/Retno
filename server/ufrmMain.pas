...
uses
  ...
  uServerDatabase, uModReturTrader, uModBankCashIn, uModBarcodeUsage, uModHistoryBarang;
...
var
  lDOB: TModHistoryBarang;
  sID: string;
begin
  with TCrud.Create(nil) do
  begin
    try
      lDOB := TModHistoryBarang.CreateDefault;
      sID := SaveToDBID(lDOB);
      lDOB.Free;

      with Retrieve(TModHistoryBarang.ClassName, InputBox('ID','ID',sID))do
      begin
